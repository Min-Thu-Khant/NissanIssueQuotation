...
namespace DAL_AmigoProcess.DAL
{
    ...
    public class INVOICE_INFO
    {
        ...
        string strInvoiceAllocationCheck = @"SELECT COUNT(*) FROM INVOICE_INFO 
                                            WHERE substring(YEAR_MONTH,2,4) = @YEAR_MONTH
                                            AND IsNull(ALLOCATION_TOTAL_AMOUNT,0) > 0";
        ...
        #region InvoiceAllocationAmountCheck
        public int InvoiceAllocationAmountCheck(string YEAR_MONTH, out string strMsg)
        {
            ConnectionMaster oMaster = new ConnectionMaster(strConnectionString, strInvoiceAllocationCheck);
            oMaster.crudCommand.Parameters.Add(new SqlParameter("@YEAR_MONTH", YEAR_MONTH));
